...
This section helps you quickly get started with the `Qwen Image Edit` model on the Qubrid AI inferencing platform.
...
Once authenticated with your API key, you can send inference requests to the `Qwen Image Edit` model and receive responses based on your input prompts.
...
`This model is suitable for inference scenarios where controlled and accurate image editing is required rather than image generation.`
...
### Summary
Qwen Image Edit is a 20B multimodal diffusion model designed for controlled image editing tasks.  
